...
original_time_series_df = pd.read_csv(root_dir + '/data/brazillian_states_time_series/RJ.csv', delimiter=";")

original_total_deaths_time_series = original_time_series_df['obitosAcumulados'].to_numpy()
original_daily_new_deaths_time_series = original_time_series_df['obitosNovos'].to_numpy()

for i in range(len(original_daily_new_deaths_time_series)):
    if original_daily_new_deaths_time_series[i] != 0:
        for j in range(i, len(original_daily_new_deaths_time_series)):
            if original_daily_new_deaths_time_series[j] == 0:
                original_daily_new_deaths_time_series[j] = 1
        break
...
adjusted_time_series_df.insert(len(adjusted_time_series_df.columns), 'Daily New Deaths',
                               original_time_series_df['obitosNovos'].to_numpy())
