...
def SIRD_model_fitting(X, *args):
    ...
    for i in range(len(forecasts_list)):
        ...
        actual_vector = time_series_df[time_series_columns_list[i]].values[30:]
...
def obtain_best_fit_estimators(y0, N, t, bounds):
    optimized_SIRD_model_result = differential_evolution(SIRD_model_fitting, bounds=bounds, args=(y0, N, t,),
                                                         maxiter=100, disp=True, polish=True, mutation=(0,1.9),workers=1)
...
if __name__ == '__main__':
    ...
    country_population_vector = np.repeat(city_population, len(time_series_df))
    ...
    S0= city_population * (1-0.0264) * 0.5 * 0.4
    y0 = S0, 10, 0, 0, 0, 0, 0
